...
# ╔═╡ 4c48524b-74f6-4f02-93e5-2132701fa578
using DimensionalData

# ╔═╡ e2dd6248-0889-4c32-b154-400d5f58a43a
using CairoMakie, AlgebraOfGraphics

# ╔═╡ b3cdb8d2-4090-4052-9034-25aa251090e4
using HTTP: HTTP

# ╔═╡ fdd15487-d783-4331-adc6-227e58af1e50
using CSV: CSV

# ╔═╡ 1baf7781-6279-44c5-8b24-f8248ef68e20
using TableScraper: TableScraper

# ╔═╡ 97b00bcb-d1e7-407a-8995-f4c90aa0485f
using DataFrames, DataFrameMacros, Chain

# ╔═╡ 59701a72-9847-411e-8845-19c0eeae9d66
using Statistics: mean

# ╔═╡ fa46b49d-ea07-4294-a676-2c910a0c84bb
using PlutoUI

# ╔═╡ e95c80be-ae34-4bb5-b8a6-70e6b41794a8
md"""
`demographics-bequests.jl` | **Version 0.1** | _last updated: March 28 2025_
"""

# ╔═╡ 81fbbda4-0ba0-11f0-2223-7586b4e5e5cc
md"""
# Assignment 4: Demographics and Bequests
"""

# ╔═╡ a985153e-cb71-4fbf-a495-21647f3c0eaa
md"""
## Tasks
* J-period model with/without mortality risk, with and without annuities
  * Compare wealth profile
* Bequests
"""

# ╔═╡ 33fe1880-ad5d-4082-a772-51c0ea8dd351


# ╔═╡ 31f5dbc6-830c-487c-a967-f56c647ec4b7
life_table₀ = let
	tbl = TableScraper.scrape_tables(
		"https://www.ssa.gov/oact/STATS/table4c6.html"
	) |> only

	colnames = identity.(tbl.columnnames)
	colnames = colnames[[1; 8:end]]
	colnames = strip.(replace.(colnames, r"\s+" => " ", r"(1|a|b)$" => ""))

	colnames[2:4] .*= " m"
	colnames[5:7] .*= " f"
	
	rows = tbl.rows[begin:end-1]
	
	csv = tempname()
	
	tbl = vcat((permutedims(r) for r ∈ rows)...)
	# create DataFrame
	df₀ = DataFrame(tbl, colnames) 
	# in order to convert strings into numbers, we use the capabilities of the CSV package
	CSV.write(csv, df₀)
	# parse CSV, now all columns have the right type
	df = CSV.read(csv, DataFrame, groupmark = ',')
end

# ╔═╡ 921365c8-d54a-4bc1-9117-75ffbed581e5
surv_df = @chain life_table₀ begin
	rename("Exact age" => "age")
	stack(Not(:age))
	@transform!(
		:sex = :variable[end] == 'm' ? "male" : "female",
		:variable = replace(:variable, r" (f|m)$" => "")
	)
	unstack(:sex, :value)
	#@transform(:average = mean([:male, :female]))
	#@subset(:variable == "Death probability")
	#select(Not(:variable))
	#@transform(:survival = 1 - :average)
	#@transform(:cum_survival = @bycol(cumprod(:survival)))
end

# ╔═╡ 9c9b9cbd-5934-439c-a5b6-252fe5239ac3
let
	j_min, j_max = extrema(life_table₀."Exact age")
	j_dim = Dim{:j}(j_min:j_max)
	
	#lives     = DimVector(life_table₀."Number of lives m", j_dim, name = :lives)
	mortality = DimVector(life_table₀."Death probability f", j_dim, name = :mortality_f)
	mortality_m = DimVector(life_table₀."Death probability m", j_dim, name = :mortality_m)

	#lines(mortality_m)
	#lines!(mortality_f)

	current_figure()
	#DataFrame(DimStack(lives, mortality))
	survival = 1 .- mortality
	cum_survival = cumprod(survival)

	lines(cum_survival)
	#lives = round.(Int, cum_survival .* 100_000)
	
end

# ╔═╡ 0a39f87a-b77f-4a14-90e4-47b10eb22b9d
url_eurostat = "https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/demo_mlifetable?format=TSV&compressed=true"

# ╔═╡ 98fd6d60-112b-494f-8328-90133c453e68
CSV.File(
	HTTP.get(url_eurostat).body,
	delim = '\t', missingstring = ":"
)

# ╔═╡ 5859f42e-303d-4cb7-a29e-bb5ebae2b1aa
raw_eurostat = DataFrame(
	CSV.File("/Users/fabiangreimel/Downloads/estat_demo_mlifetable.tsv", missingstring = [":"]),
)

# ╔═╡ bed632fa-106c-4b9f-a3ad-e46575053cf3
df_left = DataFrame(
	vcat(
		permutedims.(split.(raw_eurostat."freq,indic_de,sex,age,geo\\TIME_PERIOD", ","))...
	),
	["freq", "indic_de", "sex", "age", "geo"]
)	

# ╔═╡ 18e8e8db-fa1e-4fde-803a-f7e92c7b6284
@chain df_left begin
	@transform!(
		@subset(:age == "Y_LT1"),
		:age = "Y0"
	...
	@transform(
		:age = parse(Int, :age[2:end])
	)
end

# ╔═╡ 274e9d95-d804-45e5-9da6-09bb348a1020
unique(df_left.age)

# ╔═╡ 0f5ed075-2557-4ddc-a5e0-0d943701fa69
md"""
# Appendix
"""

# ╔═╡ b4c68eaf-a4a4-419a-82cf-931de1e3aa41
TableOfContents()
...
manifest_format = "2.0"
project_hash = "def7d630d0377c29e41099b6d72a0d007bd5152c"

...
# ╔═╡ a92e41f1-e307-4686-a42d-75677e23114f
md"""
## Parameters
"""
...
# ╔═╡ 2618b8f8-40c3-464a-b9b0-9b2a719f2767
Δr_fixed = - 0.01
...
# ╔═╡ c3280eba-21cd-11f1-02bc-6b07c173148a
md"""
...
function solve_backwards_EGM!(container, a_grid, par, prices, income)
	...
	get_cⱼ₋₁(cⱼ)       = cⱼ * (β * (1+r))^(-1/γ)
	get_aⱼ₋₁(aⱼ, cⱼ₋₁) = (cⱼ₋₁ + aⱼ - inc)/(1+r)
	
	for j ∈ (J-1):-1:1
		aⱼ_endo = a_grid
		cⱼ = consumption[a = At(a_grid), j = At(j)]
		
		cⱼ₋₁_endo = get_cⱼ₋₁.(cⱼ) # consumption on endegenous grid
		aⱼ₋₁ = get_aⱼ₋₁.(aⱼ_endo, cⱼ₋₁_endo)

		cⱼ₋₁_itp = linear_interpolation(aⱼ₋₁, cⱼ₋₁_endo)
		aⱼ_itp   = linear_interpolation(aⱼ₋₁, aⱼ_endo)
		
		cⱼ₋₁_exo = cⱼ₋₁_itp.(a_grid) # consumption on exogenous grid
		aⱼ_exo   = aⱼ_itp.(a_grid)
		
		# save the best one
		#value[      a = At(a_grid), j = At(j-1)] .= v
...
function solve_forward(container_backward, a_grid, par, income)
	...
	for j ∈ 0:J-1
		...
		# ADD COMMENT
		cⱼ_itp = linear_interpolation(a_grid, consumption[j = At(j), a = At(a_grid)])
		aⱼ_itp = linear_interpolation(a_grid, savings[    j = At(j), a = At(a_grid)])
		
		c_path[j = At(j)] = cⱼ_itp(aⱼ₋₁)
		a_path[j = At(j)] = aⱼ_itp(aⱼ₋₁)
		# ADD COMMENT
		#a_next            = savings[j = At(j), a = At(aⱼ₋₁)]
...
function solve_household(a_grid, par, prices, income; method = :VFI)
...
# ╔═╡ 53913a20-8ca7-4c7f-bd01-f849693dee4c
solution_VFI = let
	aₘᵢₙ = -0.055
	aₘₐₓ = 0.1
	n_gridpoints = 150
	...
	solve_household(a_grid, par, prices, income; method = :VFI)
end

# ╔═╡ 1b0b7834-d976-4dd6-bb22-cddbefed9a92
solution_EGM = let
	aₘᵢₙ = -0.055
	aₘₐₓ = 0.1
	n_gridpoints = 10
	...
	solve_household(a_grid, par, prices, income; method = :EGM)
end

# ╔═╡ 40a06ea7-a732-4e94-a518-2f9a059b08dd
let
	df = vcat(
		DataFrame(solution_VFI.sol_fw),
		DataFrame(solution_EGM.sol_fw),
		DataFrame(out_by_hand),
		source = :method => ["VFI (Tasks 2 & 3)", "EGM", "By hand (Task 1)"]
	)


	@chain df begin
		stack(Not([:method, :j]))
		data(_) * mapping(:j, :value, layout = :variable, color = :method, linestyle = :method) * visual(ScatterLines)
		draw(
...
manifest_format = "2.0"
project_hash = "7c6737928a8eca6350cd1e5cf6608deb92e22ca6"

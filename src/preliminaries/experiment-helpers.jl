...
# ╔═╡ 21cd224f-2fc9-4bdb-9e57-537fb0bd587f
using Distributions

# ╔═╡ 3247186a-5716-4224-b10e-321c29388163
using PlutoUI

# ╔═╡ c6d491a9-1cdd-4d38-b038-10ad740f0f19
using QuantEcon: stationary_distributions

# ╔═╡ a963c078-1f99-4fae-86c6-7191f1346759
using Interpolations: linear_interpolation

# ╔═╡ 717ff025-17b7-4f92-b525-6d4dc1265148
using PlutoLinks: ingredients

# ╔═╡ 68d2b298-1840-4205-a346-6de8569db70d
using DimensionalData

# ╔═╡ 4fc473bb-aaa9-43e4-83ee-1dd54e001bf8
using Chain, DataFrameMacros, DataFrames

# ╔═╡ dbfffc29-b94f-4fe9-8ac2-c31dba524615
using CairoMakie, AlgebraOfGraphics

# ╔═╡ 70b8a864-a032-4687-a40a-16f6910872ef
function extended_sim_df(out_trans, out_stat)
	sim_df_SS = @chain out_stat.sim_df begin
		@transform(:π_final = @bycol :π_pop ./ sum(:π_pop))
		@subset!(:π_final > 0)
	end

	sim_df_pre = @chain out_trans.sim_df begin
		@select(:j, :born)
		unique
		@transform!(:t = :j + :born)
		@subset!(:t < 0)
		leftjoin(sim_df_SS, on = :j)
	end

	sim_df_post = @chain out_trans.sim_df begin
		@transform(:t = :j + :born)
		@subset!(:t ≥ 0)
		@subset!(:π_final > 0)
	end

	keep = names(sim_df_pre) ∩ names(sim_df_post)
	
	big_df = [select!(sim_df_pre, keep...); select!(sim_df_post, keep...)]

	disallowmissing!(big_df)
end

# ╔═╡ 0fdf637d-1426-4366-ae20-d0634b51eba7
guesses_18_bequests = 
(; 
  stationary = (K_supply = 9.519414135672795, H_hh = 1.545175080379681, L_eff = 6.182668404251422),
  transition = DimStack(
      DimVector([
      9.519414137096337, 9.511745505682294, 9.523892051595219, 9.544568884623619, 9.567486135795074, 9.589885887308531, 9.610417959563046, 9.62846287804107, 9.643833653853523, 9.6565996933064, 9.666999201124316, 9.675341673565418, 9.681961543439186, 9.687170097549647, 9.691236679102225, 9.694392640632032, 9.69682801705697, 9.698692375421, 9.700100736773951, 9.701126657351029, 9.701873371898477, 9.702416104970608, 9.702809955320363, 9.703095656614437, 9.703303076810164, 9.703453947000636, 9.703563965419537, 9.703644442268471, 9.70370356686291, 9.703747357580616, 9.703780371282143
    ], Dim{:t}(0:30), name = :K_supply),
      DimVector([
      1.5437526360704568, 1.544050299320129, 1.5453758780469307, 1.5471593341963048, 1.5490437784865339, 1.5508374992829106, 1.55245325465103, 1.5538622091257106, 1.5550648643128082, 1.5560751171044211, 1.556911428144503, 1.5575935991345036, 1.5581417013845922, 1.558575662397133, 1.5589147740894134, 1.5591768089660731, 1.5593771680441644, 1.5595285666061165, 1.559641325963683, 1.5597248993766186, 1.5597866279348804, 1.5598320611359429, 1.5598653701398835, 1.559889689905237, 1.559907373085595, 1.5599201733073789, 1.5599293794437927, 1.5599359145084182, 1.5599404062819842, 1.5599432296665743, 1.5599445154031213
    ], Dim{:t}(0:30), name = :H_hh),
      DimVector([
      6.1826684042450655, 6.213228325896614, 6.234429876706191, 6.249303013811126, 6.2598516555514925, 6.267384069069733, 6.272790818848236, 6.27668065988493, 6.279477420696409, 6.281497005037066, 6.282948654062423, 6.283983203085155, 6.28471049413964, 6.285213638662068, 6.285552601049068, 6.285769372959591, 6.285892543142722, 6.285943597400415, 6.285943597400417, 6.285943597400433, 6.28594359740042, 6.285943597400433, 6.285943597400435, 6.285943597400433, 6.285943597400426, 6.285943597400433, 6.285943597400436, 6.28594359740043, 6.285943597400429, 6.28594359740044, 6.285943597400426
    ], Dim{:t}(0:30), name = :L_eff),
  ),
  inheritances_θ = [0.8846093491718385, 1.7634192203324597, 3.7386321368619733],
inheritances_θt = [
  0.8846093491718834 0.8165793660401324 0.83138183872423 0.8409790101484795 0.8470313511983929 0.8507548391573159 0.8529478710261655 0.854226347540688 0.8550522277009875 0.8557199596087703 0.8564347586181134 0.8572229928332229 0.8580217711102778 0.8587494305189578 0.8593422291914968 0.8597848405559432 0.8601084480871295 0.860353181428188 0.8605447157662294 0.8606531187730182 0.8607473355272832 0.8608279322452026 0.8608946606664365 0.8609478605995595 0.8609888495803547 0.8610196702232442 0.8610426042200838 0.8610597503338691 0.8610727887090399 0.8610829273887699 0.8610909630606431;
  1.7634192203325139 1.6286101153435109 1.6595446340802336 1.6798260025871787 1.6928731154339365 1.7011798375673388 1.7063490138223392 1.7095846671300978 1.7117679778057042 1.7134508436875278 1.71504312416431 1.7166322615706422 1.7181412409033416 1.7194565075768398 1.7205008671253106 1.721270906156417 1.721828002558029 1.7222409257443143 1.7225507095107093 1.7226842751601459 1.7227978629201652 1.7228930820278976 1.7229701025967175 1.723029650651526 1.7230736154698305 1.7231047911969894 1.7231262791508977 1.723140944578759 1.7231510902598668 1.7231583607849696 1.723163795987496;
  3.7386321368620363 3.4540161484239995 3.5219839792565715 3.5669633473645925 3.596311163426277 3.6154608620675637 3.6278330380148525 3.635916276853222 3.641483562790741 3.6456308678681477 3.6492150998637634 3.652509668999274 3.655456598238671 3.6579284949681696 3.65985089052681 3.661262083122629 3.662287621499828 3.663048083471667 3.66360233534268 3.663758070352967 3.663885085494533 3.6639877469864537 3.6640674853999196 3.664125844131414 3.664165495422344 3.664190102334678 3.6642036820698047 3.6642099716348424 3.6642119874883283 3.6642118882499055 3.664210972397096
 ]

)

# ╔═╡ c886886e-7791-4e46-8c90-363cee60af5e
guesses_18_noh_05 = 
(; 
...
# ╔═╡ bb413fbe-fc1c-4353-a879-7e5b112e7989
md"""
## Functionality
"""

# ╔═╡ 7c695693-69bb-412e-9426-71a40ca91e1e
F_marcelo = [ 
...
# ╔═╡ e82a7083-1a9f-4758-9627-d374d70ff7d0
md"""
## Saving solutions
"""

# ╔═╡ 193a0bea-f9b7-41af-a625-a60056536b54
...
# ╔═╡ 754f40e9-83b4-46a9-8495-fef02e716ecd
...
# ╔═╡ 64098ab9-0c3c-421c-b34b-bc6329c3fd41
md"""
# Appendix
"""

# ╔═╡ ece03391-48e7-4074-8fa4-b592a656a38f
TableOfContents()

# ╔═╡ 589ae0da-9d06-4bf9-a77f-e4114ee5fa42
md"""
## Local packages
"""

# ╔═╡ 9661a7b4-7e5b-4870-9442-7d85f2ea4faf
Demographics = ingredients("./demographics.jl")

# ╔═╡ 5bd193c7-0d38-4101-a572-716f6b481ea6
(; get_demographics_supersimple) = Demographics

# ╔═╡ e12555fa-e446-4a7b-8214-31e166af78aa
MoreDemographics = ingredients("./more-demographics.jl")

# ╔═╡ b8739f7c-b835-46c6-aaba-94a2e14ec560
(; get_demographics) = MoreDemographics

# ╔═╡ abcfecad-f688-4f3d-88cb-d7e1ee84729a
EGMHousingRisk = ingredients("./egm-housing-risk.jl")

# ╔═╡ 4498f9ac-b2c8-4750-9b02-4ec393f013ab
AdjustPeriod = ingredients("./adjust-period.jl")

# ╔═╡ 8def0099-b52f-4137-b920-05e92bec6c86
import PlutoTest

# ╔═╡ 925415bf-b649-4e7d-9da8-b5062b83f7a8
import StatsBase, Roots, Interpolations, QuantEcon

# ╔═╡ 72d6bc5e-07fb-49e6-9223-822dbbc2eaa5
md"""
### imports for running `out_18_bequests`
"""

# ╔═╡ 9db4e464-c739-4e3b-b9f4-5333978b40c7
(; HousingModel, stationary_GE, stationary_PE, transition_PE, transition_GE) = EGMHousingRisk

# ╔═╡ 2a84e581-1607-4c9c-ac3a-9439a2dff70b
...
# ╔═╡ 08cc3020-cdfd-4bc5-a8bf-2f63955d2eba
...
# ╔═╡ dd854b14-e7f0-4451-8ff7-a61d311510ca
...
# ╔═╡ 6d12c718-811f-42fd-ad1f-882fa1eec063
...
# ╔═╡ 2dda7f58-81c0-408f-935a-f0ff45c6f895
md"""
## Move to EGMHousingRisk
"""

# ╔═╡ 1a32620b-3d71-44e4-9801-486b09f1ee25
...
# ╔═╡ 9b1c67b7-9369-4453-bcda-b3252673d0ea
...
# ╔═╡ b658576f-79dd-42d1-99f7-e4837998c4c3
function transition_test(; old_profiles=false, amax = 200, na = 100, risk = true, ξ = 0.15, guesses_trans=nothing, tol_stat = 1e-4, tol_trans = 1e-4, T̃ = 30,
						 λ_trans = 0.02, λ_inherit = 1.0, bequests = false, skip_transition = false, details = 20, 
						 maxiter_GE = 100, maxiter_trans = 400, PE = true,
						 scenario = :supersimple, scale_m = 0.9,
						 extend_sim_df = false,
						)
...
#		return (; par, out = (; GE₀), period, statespace)
	...
	demographics_transition = if scenario == :supersimple
		get_demographics_supersimple(m₀, T̃; scale_m)
	else
	...
	out = transition_GE(Mo, T̃, par, statespace, demographics_transition, GE₀, guessed_paths;
						inheritances_θt_guess, bequests,
						details, λ = λ_trans, maxiter = maxiter_trans, tol = tol_trans, λ_inh=λ_inherit
						)

	tmp = (; par, out=out.out_PE, GE₀_etc, out_full = out, period, statespace, demographics_transition)
	
	if extend_sim_df
		sim_df_x = extended_sim_df(out.out_PE, GE₀_etc.out_PE)
		(; tmp..., sim_df_x)
	else
		tmp
	end
...
# ╔═╡ 361af26a-6612-434b-b960-44fb0477a0a7
# ╠═╡ skip_as_script = true
#=╠═╡
out_18_bequests = transition_test(old_profiles=true, guesses_trans = guesses_18_bequests,
								  ...
								  λ_trans = 0.5, λ_inherit = 0.8,
								  bequests = true, skip_transition = false, 
								  details = 1, extend_sim_df = true)
  ╠═╡ =#

# ╔═╡ 679a019d-ffee-458c-b56c-b5ca06504cf8
# ╠═╡ skip_as_script = true
#=╠═╡
out_18_noh_05 = transition_test(; old_profiles=true, guesses_trans=guesses_18_noh_05,
								amax = 200, #na = 100,
								tol_stat = 1e-10, 
								tol_trans = 1e-4, ξ = 0.0, λ_trans = 0.2, λ_inherit = 0.8, scale_m = 0.5, bequests = true, skip_transition = false, PE = false,
								maxiter_GE = 100, 
								maxiter_trans = 100, details = 1
							   )
  ╠═╡ =#

# ╔═╡ 859e4e7d-866c-4804-a3e2-576f0faaf2dd
md"""
## Packages
"""

# ╔═╡ b4ece3f1-6cf6-489a-8c47-2d2a9da4b3e8
const DD = DimensionalData

# ╔═╡ f4e58675-7fd8-43f6-bad9-e1b2c6da51d7
...
# ╔═╡ 9e65b31f-9743-42a1-8202-8136f9c5b81f
...
# ╔═╡ c17fc84b-457d-48ac-b859-e1bb594a0a87
#=╠═╡
...
manifest_format = "2.0"
project_hash = "723319901174e4df8e22adb324d3c8a91cce8a93"

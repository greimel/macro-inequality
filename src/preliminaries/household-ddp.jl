...
# ╔═╡ dc6e2076-4cd9-413c-9ab7-010f452c80b9
using PlutoUI

# ╔═╡ b68f8819-cfb1-4203-872f-d3d74aa2dceb
using QuantEcon: QuantEcon, MarkovChain, stationary_distributions, DiscreteDP, PFI

# ╔═╡ 6ef81df2-570a-4af8-b461-7aa35315c7f5
using Chain: @chain
...
md"""
# Infinitely-lived household with risky incomes
"""

# ╔═╡ 289aca64-5913-40f6-aa09-31eb0611331d
md"""
# Households' problem
"""

# ╔═╡ 86e5ce1d-ef6a-4404-8740-0125f6a9d90b
md"""
```math
\begin{align}
&\max \operatorname{E}_0\Bigl(\sum_{j=0}^\infty \beta^j u(c_j) \Bigr) \\
&\begin{aligned}
	\text{subject to } 
		&c_j + a_{j+1} = a_{j}(1 + r) + y_j \cdot w \\
		&\log(y_j) \sim \text{some Markov Chain} \\
		&y_0, a_0 \text{ given}
\end{aligned}
\end{align}
```
...
* initial state ``(y_0, a_0)``
"""

# ╔═╡ 21d77436-ff96-4832-b1d4-06625155cbcf
md"""
## Model parameters
"""

# ╔═╡ 3cb20132-759c-4441-95cd-2dcd007e6a37
md"""
...
# ╔═╡ 05f84921-c472-466b-a8b2-a41bb39be983
md"""
...
# ╔═╡ f1ceb263-fd6a-45e2-9173-b22d12450cd0
md"""
# Appendix: Using `DiscreteDP` from `QuantEcon.jl`
"""

# ╔═╡ 5a9b6b16-9aaf-49de-9fa8-32f48a7dcf3d
md"""
## Reformulating the household's problem

To find a solution to the households' problem, we will use the `QuantEcon` toolbox. The toolbox requires us to specify the household's problem in the following form

```math
\begin{align}
v(\bar s) = \max_{\bar a \in A} r(\bar s, \bar a) + \beta \cdot \mathbb{E}(v(\bar s')|\bar s, \bar a),
\end{align}
```
where $\bar s$ is the state, $\bar a$ is the action, and $r$ is the reward function.
"""

# ╔═╡ 6cd47b70-2017-4f2a-88a5-67f08d581797
md"""
For our household problem,

* the **state** ``\bar s = (a, y)`` is given by an asset level ``a`` and productivity level ``y``.
* the **action** ``\bar a = (a')`` is given by the savings ``a'``
* the **reward** ``r(\bar s, \bar a; \cdots)`` is defined as

```math
r(\bar s, \bar a; \cdots) = u(c(\bar s,\bar a;\cdots)),
```
where ``c(\bar s, \bar a;\cdots) = y \cdot w + a\cdot(1 + r) - a'``.
"""

# ╔═╡ f42cef1d-5d60-4e08-a59f-7d47924f4570
md"""
We need to specify 
1. an $n \times m$ array $R$ that contains the value of the reward function for each state ``\bar s`` and action ``\bar a`` 
$$R_{ij} = r(s_i, a_j)$$
2. an $n \times m \times n$ array $Q$ that gives the probability to end up in state $\bar s'$ in the next period for a given state $\bar s$ and action $\bar a$ in this period 
$$Q_{ijk} = \Pr(\bar s'=s_k\mathop{|}\bar s=s_i, \bar a=a_j)$$
...
# ╔═╡ e077d2ea-a1de-4267-8af6-bcc41e2c38b4
md"""
### 0. Parameters
"""

# ╔═╡ 36985842-1c8c-416b-b25f-77afe9e962f9
md"""
...
# ╔═╡ 977f628b-363e-47b2-8a69-f111bb5441c9
function Household(; σ = 1.0, β = 0.96,	
                      u = σ == 1 ? log : x -> (x^(1 - σ) - 1) / (1 - σ))
	(; β, u)
end

# ╔═╡ c831d5fe-6c30-421e-a7fd-1154b9d83dcb
hh = Household(σ = 2.0, β = 0.96)

# ╔═╡ 70e08dce-a358-4d59-b449-7708ed61bb56
md"""
### 1. State space
"""

# ╔═╡ 614ef7f6-9198-4be6-a329-0869aecf47cd
md"""
...
# ╔═╡ a751d533-2f66-4e62-aee7-d5a43e57893e
function statespace(;
...
# ╔═╡ 42c7d19d-5a7a-420d-a4c1-0300a9af30ce
ss = statespace(; a_vals = range(-1., 5., length = 200), y_chain);

# ╔═╡ 09476a19-436b-45e6-aa40-44e5e9ff3685
md"""
### 2. Computing the reward array ``R``
"""

# ╔═╡ 2efa1864-e5b8-464b-943e-7bf1e67f4752
md"""
...
# ╔═╡ c2318148-f5b2-4db2-a035-f14926f9908e
function consumption((; y, a), (; a_next), (; r, w, Δr))
	if a < 0 && Δr > 0
		r = r + Δr
	end
	c = w * y + (1+r) * a - a_next
...
# ╔═╡ 93216447-aa66-4be4-9247-5e913f2914d2
function solve_details(ddp, states, policies, prices; solver = PFI)
...
# ╔═╡ 89853080-9f14-453d-9bb4-dfc6c7cde4d5
...
# ╔═╡ b3aa19ba-2ee8-4815-aeb2-fd1204b969f9
...
# ╔═╡ c5937526-308e-4c9d-9c89-6b850fe27702
function setup_R(states, policies, prices, u)
	R = zeros(length(states), length(policies))
	setup_R!(R, states, policies, prices, u)
end

# ╔═╡ ccedd9db-d8fd-498c-84b7-df7333ffd4f1
md"""
### 2. Transition probabilities ``Q``
"""

# ╔═╡ 341461b5-dffe-4c48-9a31-9b11291b6343
md"""
...
# ╔═╡ 58ede7ff-4bcc-4b82-82be-efe0ca61eb19
...
# ╔═╡ 50eb4ad5-7078-477c-a2ab-16eea4d28263
...
function setup_DDP(household, statespace, prices)
	(; β, u) = household
	(; states, policies, states_indices, policies_indices, y_chain) = statespace
...
manifest_format = "2.0"
project_hash = "c4cf0984c34132fd026de4a5e8fa2aa4e0a4ccb2"

...
function choicesⱼ₋₁(::HousingModel, stateⱼ₋₁, stateⱼ, 𝔼valⱼ, constrainedⱼ₋₁, εⱼ₋₁,
	...
	uⱼ₋₁ = uu(cⱼ₋₁, hⱼ₋₁, (; ξ, par.σ))
	v_bⱼ₋₁ = par.v(aⱼ) # bequest utility, rename to v later, TODO should really by stateⱼ ≠ aⱼ
	valⱼ₋₁ = uⱼ₋₁ + βⱼ_over_βⱼ₋₁ * (1 - ⱼ₋₁.m) * 𝔼valⱼ # value
	#u + m * v || vs || u + beta * m * v
	...
	stuff = (; co=cⱼ₋₁, ho=hⱼ₋₁, 
			 ...
			 z_next=zⱼ, ℓ_eff = ℓ_effⱼ₋₁, inheritance = inhⱼ₋₁, income = incⱼ₋₁, z = zⱼ₋₁, ⱼ₋₁.m, #=h_lc = ⱼ₋₁.h,=# value = valⱼ₋₁, next_value =  𝔼valⱼ,  utility = uⱼ₋₁, β_over_β = βⱼ_over_βⱼ₋₁, constrained = constr2)

	(; c = stuff.co, v=valⱼ₋₁, stuff)
end
...
function simulate_cohorts(Mo, par, permanent, statespace, demographics_transition, GE_sol_perm; price_paths, j_last = par.J, T̃, inheritances_tj)
	...
	(; m_jborn, π_jt) = demographics_transition 
...
# ╔═╡ c45f354b-f9e1-4d78-80c6-7ad11666f5e0

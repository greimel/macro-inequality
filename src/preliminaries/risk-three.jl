...
# ╔═╡ 592bede8-fe12-4fa9-a91e-cc9f0c80abc1
using CategoricalArrays: cut

# ╔═╡ a0e99c67-fc34-4edf-b622-a441445fb839
using StatsBase: weights

# ╔═╡ 24d686c3-a80e-4861-acb3-e36f7829c3f8
using Statistics: mean

# ╔═╡ 82eef77a-b6bf-4f7b-aa4f-4c4d5931052a
using StatsBase: quantile

# ╔═╡ 42b5acfe-f681-45e6-9d1f-1d640668c3ca
using Random

# ╔═╡ 3af7a53c-20f8-429a-9015-b8c2dd6e6470
using PlutoTest

# ╔═╡ bd4f060e-49dd-4652-8244-2bced0c53efe
using DataFrames, DataFrameMacros, Chain, DimensionalData

# ╔═╡ 67ca73c6-c187-4d82-a36e-321ee6db2594
using CairoMakie, AlgebraOfGraphics

# ╔═╡ d442bd03-b03f-4739-a610-39185b32d307
using PlutoUI: TableOfContents

# ╔═╡ aa87c159-4407-4a01-9fc5-e3b7d673fe00
using PlutoLinks: ingredients

# ╔═╡ c65315b6-1963-11f1-3b0f-e93e3b6536bf
md"""
# Three-period model with risk
"""

# ╔═╡ 44e640e3-2c36-4f5a-8a69-1819e66f7d47
md"""
# Simulate a household
"""

# ╔═╡ da3d84f3-569b-4e78-a56d-5b85f736b431
md"""
# Appendix
"""

# ╔═╡ 122e39c2-74aa-423c-891a-44a900a444e1
import StatsBase, Roots, Interpolations, QuantEcon

# ╔═╡ 890a3bd9-3597-4af2-86f6-1cac77d0fce4
let
	mc = QuantEcon.MarkovChain([0 1; 1 0])

	QuantEcon.simulate(mc, 10, init = 1)
end

# ╔═╡ 97e9e755-c3d6-48a9-912b-a5d37fecc643
function draw_next_ε(ε_chain, ε)
    ε_path = QuantEcon.simulate(ε_chain, 2; init = ε)
    ε_path[end]
end

# ╔═╡ b2e4336c-a63e-4a84-a3ee-d460c260afbf
EGMHousingRisk = ingredients("./egm-housing-risk.jl")

# ╔═╡ 7d92ddb1-b0cb-4a37-8c47-dc5bfd2a93c0
get_cali_test(; amax = 12.0, na = 500, exponential = false) = let

	J = 4
	j_dim = Dim{:j}(0:J)
	m = DimArray([zeros(J); 1.0], j_dim, name = :m)
	h = ones(j_dim, name = :y)

	ε_chain = EGMHousingRisk.simple_income_risk()
	permanent = EGMHousingRisk.no_permanent_states()
	
	statespace = EGMHousingRisk.Statespace(; 
							amin = 0.0, amax, na, 
							ε_chain, permanent, exponential)
	
	par = EGMHousingRisk.get_par₀(; h, m, ξ = 0.0, δ = 0.1, β = 0.995,
				   bonds2GDP = 1.0, NFA2GDP = 0.0, τ = 0.0,
				   ν₀ = 0.0, annuities = false,
				   α = 0.33, θ = 1.0, Z̲ = 0.0, a̲ = 0.0)

	π_init = EGMHousingRisk.trivial_initial_distribution(statespace, init_state = 0.0)

	inheritances = EGMHousingRisk.no_inheritances(par, statespace)
	(; par, statespace, π_init, inheritances)
end

# ╔═╡ 168ca329-caa7-47d3-8f07-ac501942e1af
function interpolate_policy_1d(x, x_grid, values)
    (; low, high) = EGMHousingRisk.weighted_neighbours(x, x_grid)
    v_low  = values[state = At(low.x)]
    v_high = values[state = At(high.x)]
    (1 - high.weight) * v_low + high.weight * v_high
end

# ╔═╡ eb584a96-e2b4-4291-a500-0621e94093ba
(; stationary_PE, stationary_GE, interest_rate, wage, HousingModel) = EGMHousingRisk

# ╔═╡ 5d94f465-40af-4683-adce-e2d2b275b9f4
out_test = let
	...
	(; aggregates, prices) = out.out_PE
	(; K_supply) = aggregates.updated
	(; K_hh, ζ) = aggregates.aggregates
	(; state, c, ℓ_eff, a_next) = out.out_PE.raw_aggregates
	(; r) = prices  
...
# ╔═╡ 4a09b96a-51fb-40ca-a9ef-e8475c021f0c
let
	
	sim_df = @chain out_test.out.out_PE.sim_df begin
	
		@subset(:π_pop > 0)
	end

	wealth_grid = range(extrema(sim_df.z_next)..., length = 100)

	@chain sim_df begin
		@transform(:wealth_bin = @bycol cut(:z_next, wealth_grid, extend=true))
		@groupby(:wealth_bin)
		@combine(
			:binned_wealth = mean(:z_next, weights(:π_pop)),
			:π = sum(:π_pop)
		)
		data(_) * mapping(:binned_wealth, :π) * visual(Lines)
		draw
	end
	#=
		sort(:z_next)
		@transform(:cum_π = @bycol cumsum(:π_pop))
		data(_) * mapping(:z_next, :cum_π) * visual(Lines)
	draw
	=#
end

# ╔═╡ d59d0d2e-1e68-47a6-a1a8-fcc02621b008
let
	q = [0.25, 0.5, 0.75]
	sim_df = @chain out_test.out.out_PE.sim_df begin
		@subset(:π_pop > 0)
		...
		@combine(
			:q = q, 
			:wealth = quantile(:z_next, weights(:π_pop), [0.25, 0.5, 0.75])
			#:wealth = mean(:z_next, weights(:π_pop))
		
		)
		data(_) * mapping(:j, :wealth, color = :q => nonnumeric) * visual(Lines)
		draw
	end
...
# ╔═╡ 5c97a032-ebc9-4b8d-9414-66ab4acdc488
only(out_test.out.out_PE.sols).sol.sol_backward

# ╔═╡ 74292bc9-49ec-47a3-b92c-28a56204b2bc
let
	mc = out_test.statespace.ε_chain
	ε₀ = mc.state_values[1]

	QuantEcon.simulate(mc, 3, init=1)
end

# ╔═╡ 3234157c-519b-4ca0-82a7-cfa950da00a8
Demographics = ingredients("./demographics.jl")

# ╔═╡ 40cfc7c0-f6f4-4dd4-97c5-47251b05432a
(; get_π_j) = Demographics

# ╔═╡ 234b5feb-3f28-4dda-803f-dc34d7fa5c7c
const DD = DimensionalData

# ╔═╡ 5a2c31f5-3c5c-4284-aa7f-499a5089c6e5
function simulate_household_path(sol_backward, statespace;
        z0,
        i_ε₀ = 1,
        θ0 = nothing,
    ...
    (; grid, ε_grid, P_from, ε_chain) = statespace
	j_dim = DD.dims(sol_backward.next_state, :j)
    j0, J = extrema(j_dim)
	...
	ε_path = DimVector(
		QuantEcon.simulate(ε_chain, length(j_dim), init = i_ε₀),
		j_dim,
		name = :ε
	)
    ...
    for j in j_dim
		ε = ε_path[j = At(j)]
        #stuff_j_ε = sol_backward.stuff[j = At(j), ε = At(ε)]
        next_state_j_ε = sol_backward.next_state[j = At(j), ε = At(ε)]
        c_j_ε = sol_backward.c[j = At(j), ε = At(ε)]
        value_j_ε = sol_backward.value[j = At(j), ε = At(ε)]
        constrained_j_ε = sol_backward.constrained[j = At(j), ε = At(ε)]

        #stuff = interpolate_policy_1d(z, grid, stuff_j_ε)
        z_next = interpolate_policy_1d(z, grid, next_state_j_ε)
        c = interpolate_policy_1d(z, grid, c_j_ε)
        value = interpolate_policy_1d(z, grid, value_j_ε)
        constrained = interpolate_policy_1d(z, grid, constrained_j_ε)

        push!(rows, merge(
            (; j, z, ε, θ = θ0, z_next, c, value, constrained),
            #pairs(stuff)
        ))
...
# ╔═╡ d18f1e13-b1ee-4e08-b2a8-379ee2852676
sim_path = simulate_household_path(
    only(out_test.out.out_PE.sols).sol.sol_backward,
    out_test.statespace;
    z0 = 0.0,
    i_ε₀ = 1
...
# ╔═╡ 1c1b9171-b936-45d9-92e2-177bb4920b7b
TableOfContents()
...
manifest_format = "2.0"
project_hash = "a33731ecb408a4e2f52eb8c90ad2dbc6cda485eb"
